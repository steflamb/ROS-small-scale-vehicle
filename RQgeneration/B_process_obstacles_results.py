...
def compute_angle_between_normals(n1, n2):
    ...
    dot_product = np.clip(dot_product, -1.0, 1.0)  # for numerical safety
    return float(np.degrees(np.arccos(dot_product)))

def extract_boxes(json_path, key):
    with open(json_path, 'r') as f:
        data = json.load(f)
    return [obj["front_face_corners"] for obj in data.get(key, [])]
...
def match_boxes_by_center_and_orientation(ref_boxes, cmp_boxes):
    ...
    for i, (ref_c, ref_n) in enumerate(ref_props):
        ...
        for j, (cmp_c, cmp_n) in enumerate(cmp_props):
            ...
            score = center_dist + angle_diff * 0.01  # small weight to angle, adjust as needed
            if score < best_score:
                best_score = score
                best_j = j
                best_data = {
...
if __name__ == "__main__":
    input_folder = "/media/ast/1868aaf7-73b4-4e22-9fd8-ff4f3eb47346/ASE/PERCEPTION_INPUTS/lidar/annotations"
