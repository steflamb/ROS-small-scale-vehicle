...
for test in ["green","red"]:
    ...
    mean_dist_sim, mean_dist_mixed, mean_dist_vil = [], [], []

    if test=="green":
        exp_arrays=["success1","success2","success3","success4","success5"]
    else:
        exp_arrays=["success1","success2","success3","success4","success5","fail_on_obs1","fail_on_obs1_2","fail_after_obs_1","fail_at_second_obstacle"]

    for iteration in exp_arrays:
        if iteration in ["fail_on_obs1_2","fail_after_obs_1"]:
            iteration_vil="success5"
        elif iteration=="fail_on_obs1":
            iteration_vil="crash"
        elif iteration=="fail_at_second_obstacle":
            iteration_vil="failend"
        else:
            iteration_vil=iteration

        if iteration in ["fail_on_obs1","fail_on_obs1_2","fail_after_obs_1","fail_at_second_obstacle"]:
            iteration_sim="success5"
        else:
            iteration_sim=iteration

        fig = plt.figure()
        path = path_prefix + "real/" + test  + "_" + str(iteration) + path_suffix
        ...
        mixed_path = path_prefix + "mixed/" + test  + "_" + str(iteration) + path_suffix
        ...
        vil_path = path_prefix + "vil/" + test  + "_" + str(iteration_vil) + path_suffix
        ...
        sim_path = path_prefix + "sim/" +test +"_" + str(iteration_sim) + path_suffix
        ...
        plt.savefig(f"PROCESSED RESULTS/RQ3/MODULAR/PIPELINE/plots/{test}_{iteration}.pdf")
...
save_path = Path("PROCESSED RESULTS/RQ3/MODULAR/PIPELINE/aggregated_summary.json")
...
cte_path = Path("/media/ast/1868aaf7-73b4-4e22-9fd8-ff4f3eb47346/ASE/PROCESSED RESULTS/RQ4/MODULAR/PIPELINE/hybrid_trajectory_summary.json")
