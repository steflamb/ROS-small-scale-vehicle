...
class C:
    ...
    ROAD_WIDTH = 1.2*3    #distance from center of lane to edge of lane
    # ROAD_SAMPLE_STEP = 3  #precision of reference path
    ROAD_SAMPLE_STEP = 0.4
    ...
    MAX_STEER = 3.# [rad] maximum steering angle
...
def sampling_paths_for_Cruising(l0, l0_v, l0_a, s0, s0_v, s0_a, ref_path):
    global road_boundary
    PATHS = dict()

    # s0_v = max(s0_v, 0.1)

    for s1_v in [C.TARGET_SPEED]:

        # for t1 in np.arange(4.5, 5.5, 0.2):
        for t1 in [C.MIN_T,C.MAX_T]:
            path_pre = Path()
            ...
            for l1 in np.arange(-3*C.ROAD_WIDTH, 3*C.ROAD_WIDTH, C.ROAD_SAMPLE_STEP):
                ...
                pts = np.vstack([path.x, path.y]).T

                inside_mask = road_boundary.contains_points(pts)
                inside_ratio = np.sum(inside_mask) / len(pts)

                if inside_ratio < 0.7:  # Require at least 70% points inside
                    continue

                offroad_mask   = ~road_boundary.contains_points(pts)
                offroad_count  = offroad_mask.sum()
        
               
                offroad_cost =  offroad_count
                ...
                skip=3
                avg_offset = sum(np.abs(path.l[skip:])) / (len(path.l) - skip)
                if len(path.x)<2:
                    continue
...
def new_obstacles(msg):
    ...
    for obstacle in msg.data:
        # print(obstacle)
        obstacle_dict[obstacle.name] = [obstacle.x, obstacle.y, obstacle.yaw]
    new_array = np.array([])
    for o in obstacle_dict:
        obstacle = Obstacle(obstacle_dict[o][0], obstacle_dict[o][1], obstacle_dict[o][2], C.OBS_W,C.OBS_L)
...
def main_Crusing():
    ...
    if reverse:
        right_lane = map_data["left_margin"]
        left_lane = map_data["right_margin"]
    else:
    ...
    waypoints   = map_data["sim_waypoint_list"]
    center_line = list(map_data["sim_waypoint_list"])
    ...
    left_pts  = list(zip(bx1, by1))
    right_pts = list(zip(bx2, by2))[::-1]
    verts = np.array(left_pts + right_pts)
    ...
    obstacle_array = np.array(list(map(lambda o: Obstacle(obstacle_dict[o][0], obstacle_dict[o][1], (3.14/2),C.OBS_W,C.OBS_L), obstacle_dict.keys())))
    ...
    while not rospy.is_shutdown():
        ...
        paths_dict = sampling_paths_for_Cruising(l0, l0_v, l0_a, s0, s0_v, s0_a, ref_path)
        for trial_path in paths_dict:
            ax.plot(trial_path.x, trial_path.y,
                    linewidth=0.5,
                    color='lightgray',
                    alpha=0.7)
            
        path = extract_optimal_path(paths_dict)
# 0.5
        # # print("got new path")
        ...
        try:
            current_time = time.time()
            time_diff = current_time-starting_time
            if np.hypot(path.x[1] - rx[-1], path.y[1] - ry[-1]) <= 0.2*7.33 and time_diff>10.0:
                print("Goal")
                pub_waypoints.publish(True, [])
                pub_going.publish(False)
                ax.cla()
                # for stopping simulation with the esc key.
                ax.plot(rx, ry, linestyle=':', color='gray')
                ax.plot(cx, cy, linestyle='--', color='gold')
                ax.plot(bx1, by1, linewidth=1.5, color='k')
                ax.plot(bx2, by2, linewidth=1.5, color='k')
                
                ax.plot(path.x, path.y,linewidth=2,color='royalblue',label='optimal')
                ax.plot(obs_x, obs_y, 'ok')
                # for obs in C.obs:
                ...
                for obs in obstacle_array:
                    # 1) compute yaw_rad *exactly* as draw.draw_car expects:
                    #    (this is what you pass into draw.draw_car)
                    yaw_deg = obs.yaw
                    yaw_rad = math.radians(360-yaw_deg)
                    yaw_rad = pi_2_pi(yaw_rad)

                    # 2) draw the little car
                    draw.draw_car(obs.x, obs.y, yaw_rad, steer, C)

                    # 3) now build a Rectangle *un-rotated*, and apply an Affine2D
                    rect = Rectangle(
                        ...
                        transform=(
                            Affine2D()
                                .rotate_around(obs.x, obs.y, yaw_rad)
                            + ax.transData
                        )
                    )
                    ax.add_patch(rect)
                ...
                # polygon = mpatches.Polygon(verts, closed=True, fill=True,
                #                edgecolor='yellow', linewidth=2, linestyle='--')
                # ax.add_patch(polygon)
                ax.set_title("[Crusing Mode]  v :" + str(s0_v * 3.6)[0:4] + " km/h")
                ax.set_aspect("equal")
                fig.canvas.draw()       # re-render the figure
        ...
        ax.plot(bx1, by1, linewidth=1.5, color='k')
        ax.plot(bx2, by2, linewidth=1.5, color='k')
        ax.plot(path.x[1:], path.y[1:], linewidth='2', color='royalblue')
        ax.plot(obs_x, obs_y, 'ok')
        # polygon = mpatches.Polygon(verts, closed=True, fill=True,
        #                    edgecolor='yellow', linewidth=2, linestyle='--',fillcolor='yellow')
        # ax.add_patch(polygon)
        # for obs in C.obs:
        #     draw.draw_car(obs.x, obs.y, obs.yaw, steer, C)
...
def main_Stopping():
    ...
    for obstacle in map_data["obstacles"]:
        obstacle_dict[obstacle[0]] = [obstacle[1],obstacle[2]]
    obstacle_array = np.array(list(map(lambda o: Obstacle(obstacle_dict[o][0], obstacle_dict[o][1], (3.14/2),C.OBS_W,C.OBS_H), obstacle_dict.items())))
    ...
    while True:
        ...
        if np.hypot(path.x[1], path.y[1] - 0) <= 0.2:
            print("Goal")
            pub_waypoints.publish(True, [])
            pub_going.publish(False)
            break
        ...
        ax.set_title("[Stopping Mode]  v :" + str(s0_v * 3.6)[0:4] + " km/h")
        ax.set_aspect("equal")
