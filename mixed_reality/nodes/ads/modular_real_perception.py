...
class pointcloud_processing_node:
    def __init__(self):
        ...
        self.min_z_filter = rospy.get_param("~min_z_filter", 0.4)
        self.max_z_filter = rospy.get_param("~min_z_filter", 1)
        ...
        self.obstacle_pub = rospy.Publisher("/obstacles_lidar", Obstacles, queue_size=1)
    ...
    def pc_callback(self, msg):
        ...
        o3d_cloud = self.crop_box(o3d_cloud, xlim=(-0.5,0.6), ylim=(-10,10), zlim=(-1,2))
        o3d_cloud = self.remove_wedge(o3d_cloud,
                              angle_min_deg=0,
                              angle_max_deg=100)
        ...
        # 2) iteratively peel off the floor
        cloud_noground = self.remove_ground_planes(o3d_cloud,distance_thresh=0.03,ransac_n=3,num_iterations=700,min_inliers=5000,max_tilt_deg=90)

        # 3) publish and continue
        ros_noground = self.open3d_to_ros(cloud_noground, frame_id=self.frame_id)
        self.pub_noground.publish(ros_noground)

        if len(cloud_noground.points) < 1:
            rospy.loginfo("No points above ground; skipping clustering.")
            return

        # 3) NOW apply Z-filter only for clustering
        points_noground = np.asarray(cloud_noground.points)
