...
def simulator_node():
    ...
    while not rospy.is_shutdown():
        ...
        if pub_mix_image is None:
            pub_mix_image = rospy.Publisher("sim/image_for_mixing", SensorImage, queue_size=10)
        pub_mix_image.publish(msg_mix_image)
        if pub_road_image is None:
            pub_road_image = rospy.Publisher("sim/image_road", SensorImage, queue_size=10)
        pub_road_image.publish(msg_road_image)
        if pub_obs_image is None:
            pub_obs_image = rospy.Publisher("sim/image_obs", SensorImage, queue_size=10)
